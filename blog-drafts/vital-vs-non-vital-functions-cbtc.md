...
[FIGURE: Vital/non-vital partition diagram for a representative CBTC system, showing SIL 4 functions (MA generation, ATP supervision, localization, switch verification, door enable, vital comms) inside the safety envelope and SIL 0–2 functions (ATS dispatching, ATO regulation, PIS, NMS, diagnostics) outside, with the explicit interfaces that cross the boundary.
Source: new figure to be generated.
Caption: "Every cross-boundary interface between non-vital and vital is where safety-case attention concentrates. The vital side cannot trust non-vital inputs without explicit safety arguments."]

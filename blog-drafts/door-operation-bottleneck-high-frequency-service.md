...
[FIGURE: Timeline diagram of a station stop showing precision stop (5 sec), berthing detection (1 sec), door open cycle (4 sec), passenger dwell (25 sec), door close cycle (4 sec), MA grant and departure (2 sec) for a 41-second total, annotated with where engineering levers can compress each phase.
Source: new figure to be generated.
Caption: "Dwell as the dispatcher sees it. The 25-second passenger phase is governed by physics; the 16 seconds around it are governed by engineering choices in the procurement."]

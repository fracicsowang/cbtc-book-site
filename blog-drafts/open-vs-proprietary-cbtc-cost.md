...
[FIGURE: Procurement-mechanism layer diagram showing where each open-architecture lever applies (ICDs at all subsystem boundaries, source-code escrow at the safety-critical software layer, patent licensing across all interfaces, modular procurement at the system level, framework agreements at the program level, open APIs at the non-safety integration layer). Source: new figure to be generated. Caption: "Open architecture in US CBTC is a stack of procurement mechanisms, not a single specification."]
...
[FIGURE: Cost-benefit analysis chart showing each open-architecture mechanism (ICDs, source-code escrow, patent licensing, modular procurement, framework agreements, open APIs) plotted on agency cost (X axis) versus realized lock-in savings (Y axis). Source: new figure to be generated. Caption: "Some open-architecture mechanisms have high payoff at low cost; others require substantial agency capability to realize."]

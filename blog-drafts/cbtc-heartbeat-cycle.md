...
[FIGURE: Latency budget bar chart showing the four end-to-end stages (VOBC processing, uplink, ZC processing, downlink) with allocated budgets and tolerance bands, plus the communication loss timeout overlay.
Source: new figure to be generated.
Caption: "End-to-end latency budget across the four heartbeat phases. Cycles that exceed the budget force wider position-uncertainty margins; cycles that miss the communication loss timeout trigger safe stopping."]
